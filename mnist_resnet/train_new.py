...
for loop_idx in range(args.total_loops):
    ...
    def test(epoch, testloader, train_eval_loader):
        ...
        if acc > best_acc:
            ...
            torch.save(state, folder_savemodel + f'/ckpt{loop_idx+1}.pth')
    ...
    for itr in range(5 * batches_per_epoch):
        ...
        if itr % batches_per_epoch == 0:
            ...
            with torch.no_grad():
                ...
                if val_acc > best_acc:
                    torch.save({'state_dict': model.state_dict(), 'args': args}, os.path.join(savefolder_fc, f'model{loop_idx+1}.pth'))
                    best_acc = val_acc
                print(
